...
public record DaySchedule(string DayLabel, List<ScheduleSlot> Slots);
public record SuburbSchedule(Suburb Suburb, List<DaySchedule> Days, DateTime FetchedAt);
...
internal class GridParser
{
	...
	internal GridParser(ILogger<GridParser> logger) => _logger = logger;

	internal async Task<SuburbSchedule> ParseScheduleAsync(
		Suburb suburb,
		int stage,
		string html,
	...
	internal static (DateTime startUtc, DateTime endUtc) ToUtcSlot(
	 DateOnly date,
	 TimeOnly start,
	 TimeOnly end)
	{
